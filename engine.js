...
function drawTile(ctx,t,tx,ty,ts){
  ...
  // Tall grass overlay — always draw distinct marker regardless of map
  if(t===TILE.TALL){
    ctx.fillStyle='rgba(0,60,0,0.45)';
    ctx.fillRect(tx,ty,ts,ts);
    ctx.fillStyle='rgba(0,200,60,0.55)';
    const b=Math.floor(ts/5);
    ctx.fillRect(tx+b,ty,b,ts);
    ctx.fillRect(tx+b*3,ty,b,ts);
  }
