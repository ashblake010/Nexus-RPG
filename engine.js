...
// Buildings: {x, y, key, w, h} — tile position + building image key + tile footprint
const BUILDINGS_BY_MAP = {
  bootville: [
    {x:7, y:3, key:'lab',    w:3, h:3},  // Prof Ada's lab
  ],
  gridlock: [
    {x:7, y:2, key:'gym',    w:3, h:3},  // Gridlock Gym
    {x:7, y:4, key:'center', w:3, h:3},  // Pokemon Center
    {x:19,y:4, key:'shop',   w:2, h:2},  // Shop
  ],
  route2: [],
  ironhaven: [
    {x:7, y:2, key:'gym',    w:3, h:3},  // Ironhaven Gym
    {x:7, y:4, key:'center', w:3, h:3},  // Pokemon Center
    {x:19,y:4, key:'shop',   w:2, h:2},  // Shop
  ],
};
...
function drawTile(ctx,t,tx,ty,ts){
  ...
  // Exits get cyan overlay on top of whatever tile
  if(t===TILE.EXIT_N||t===TILE.EXIT_S){
    ctx.fillStyle='#00ffff18';ctx.fillRect(tx,ty,ts,ts);
    ctx.strokeStyle='#00ffff55';ctx.lineWidth=1;
...
function drawMap(){
  ...
  // Buildings — drawn over tiles, sized to cover their footprint
  for(const b of getBuildingsForMap(G.map)){
    const bx=b.x*TILE_SIZE+offX;
    const by=b.y*TILE_SIZE+offY;
...
function canWalk(x,y){
  ...
  // Block building footprints (all but front row)
  for(const b of getBuildingsForMap(G.map)){
...
function startBattle(enemy,isTrainer,isGym){
  ...
  if(typeof BATTLE_BGS!=='undefined'){
    const bgKey=isGym?'gym':isTrainer?'trainer':
      (G.map==='route2'?'route2':G.map==='ironhaven'?'ironhaven':'wild');
