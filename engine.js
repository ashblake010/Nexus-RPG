...
function drawMap(){
  ...
  ctx.save();
  // Apply directional transform based on G.facing
  const facing=G.facing||'down';
  if(facing==='left'){
    ctx.translate(px+TILE_SIZE,py+walkBob);
    ctx.scale(-1,1);
    ctx.translate(-px,0);
  } else if(facing==='up'){
    ctx.translate(px,py+TILE_SIZE+walkBob);
    ctx.scale(1,-1);
    ctx.translate(0,-py);
  }
  // 'right' and 'down' draw normally (bob applied via py offset)
  const drawPy=py+((facing==='left'||facing==='up')?0:walkBob);
  if(typeof CHAR_SPRITES!=='undefined'&&CHAR_SPRITES.player&&CHAR_SPRITES.player.complete&&CHAR_SPRITES.player.naturalWidth>0){
    ctx.drawImage(CHAR_SPRITES.player,px,drawPy,TILE_SIZE,TILE_SIZE);
  } else {
    const FACING_EMOJI={'up':'🔼','down':'🔽','left':'◀️','right':'▶️'};
    ctx.font=Math.floor(TILE_SIZE*.65)+'px serif';
    ctx.textAlign='center';ctx.textBaseline='middle';
    ctx.fillText(FACING_EMOJI[facing]||'🧑',px+TILE_SIZE/2,drawPy+TILE_SIZE/2);
  }
  ctx.restore();
  ctx.shadowBlur=0;
}
...
function openBagInBattle(){
  ...
  if((G.bag.fullheal||0)>0){
    ...
    btn.onclick=()=>{
      ...
      restoreMovePP(bs.player);G.party[0].movePP=bs.player.movePP; // restore PP too
