...
function renderBag(){
  const items={
    nexball:  {emoji:'🔵',name:'NEX BALL',  desc:'A basic ball for catching wild Pokémon.',color:'#003a6e'},
    superball:{emoji:'🟣',name:'SUPER BALL',desc:'Better catch rate than a Nex Ball.',color:'#2a0050'},
    healpack: {emoji:'💊',name:'HEAL PACK', desc:'Restores 40 HP to one Pokémon.',color:'#003a20'},
    revive:   {emoji:'💫',name:'REVIVE',    desc:'Revives a fainted Pokémon to 50% HP.',color:'#2a2a00'},
    fullheal: {emoji:'✨',name:'FULL HEAL', desc:'Fully restores one Pokémon.',color:'#1a1a40'},
  };
  ...
  for(const [key,count] of Object.entries(G.bag)){
    ...
    const item=items[key]||{emoji:'?',name:key,desc:'',color:'#1a2a30'};
    const div=document.createElement('div');div.className='bag-item';
    div.innerHTML=`<div class="bag-item-icon" style="background:${item.color||'#0a2030'}">${item.emoji}</div><div class="bag-item-body"><div class="bag-item-name">${item.name}</div><div class="bag-item-desc">${item.desc}</div></div><div class="bag-item-count">×${count}</div>`;
...
function renderParty(){
  ...
  G.party.forEach((p,i)=>{
    ...
    const spriteHtml=typeof SPRITE_IMAGES!=='undefined'&&SPRITE_IMAGES[key]
      ?`<img src="${SPRITE_IMAGES[key]}" style="width:100%;height:100%;object-fit:contain;image-rendering:pixelated;">`
      :`<span style="font-size:36px;">${p.emoji}</span>`;
    div.innerHTML=`<div class="party-sprite">${spriteHtml}</div><div class="party-info">
      <div class="party-name">${p.name}${i===0?' <span style="color:var(--accent);font-size:8px;letter-spacing:1px;">◈ LEAD</span>':''}</div>
      <div class="party-type-badge">${p.type}</div>
      <div class="party-stats" style="margin-top:4px;">Lv.<b style="color:var(--text);">${p.level}</b> &nbsp; HP: <b style="color:${hpCol};">${p.hp}/${p.maxHp}</b></div>
