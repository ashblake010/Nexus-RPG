...
// ══════════════════════════════════════════════
// ANIMATION SYSTEM
// owAnimFrame   — requestAnimationFrame handle for overworld loop
// waterPhase    — ever-incrementing angle for water ripple sine wave
// playerWalkFrame — increments each move() call for walk bob
// ══════════════════════════════════════════════
let owAnimFrame=null, waterPhase=0, playerWalkFrame=0;

function startOwAnim(){
  if(owAnimFrame)return; // already running
  function loop(){
    waterPhase+=0.05;
    if(waterPhase>Math.PI*200)waterPhase=0; // prevent float drift
    drawMap();
    owAnimFrame=requestAnimationFrame(loop);
  }
  owAnimFrame=requestAnimationFrame(loop);
}
function stopOwAnim(){
  if(owAnimFrame){cancelAnimationFrame(owAnimFrame);owAnimFrame=null;}
}

/** Fade to dark, run callback at mid-point, then fade back in. */
function fadeTransition(callback,color='#030810'){
  const ov=document.createElement('div');
  ov.style.cssText='position:fixed;inset:0;background:'+color+';z-index:9999;pointer-events:none;opacity:0;transition:opacity 0.18s ease';
...
/** Rapid white flashes before wild battle, then fires callback. */
function wildEncounterFlash(callback){
  const ov=document.createElement('div');
  ov.style.cssText='position:fixed;inset:0;background:#ffffff;z-index:9999;pointer-events:none;opacity:0';
  document.getElementById('game-container').appendChild(ov);
  const pattern=[0,80,160,240,320,400]; // alternating flash timings
  pattern.forEach((t,i)=>{
    setTimeout(()=>{
      ov.style.opacity=(i%2===0)?'0.9':'0';
      if(i===pattern.length-1) setTimeout(()=>{ov.remove();callback();},90);
    },t);
  });
}
...
function showScreen(id){
  ...
  if(id==='overworld') startOwAnim(); else stopOwAnim();
}
...
function applyMoveEffect(effect,attackerIs,defenderIs,dmgDealt){
  ...
  // ── Stats ──
  if(effect==='def+1'||effect==='def+2'){
    const stages=effect==='def+1'?1:2;
    if(attackerIs==='player'){
      bs.playerStatusDef=(bs.playerStatusDef||0)+stages;
    } else {
      bs.enemyStatusDef=(bs.enemyStatusDef||0)+stages;
    }
    return attacker.name+"'s defense "+(stages===1?'rose!':'sharply rose!');
  }
  ...
  if(statusType&&!bs[defStatusKey]&&Math.random()<chance){
    const confTurns=(statusType==='conf')?(2+Math.floor(Math.random()*3)):0; // conf lasts 2-4 turns
    bs[defStatusKey]={type:statusType,turns:0,confTurns};
...
// ══════════════════════════════════════════════
// PP SYSTEM
// initMovePP  — fills in any missing PP entries from MOVE_DATA (safe on old saves)
// restoreMovePP — sets all PP back to max (Pokémon Center / Full Heal)
// ══════════════════════════════════════════════
function initMovePP(p){
  if(!p.movePP) p.movePP={};
  for(const mv of (p.moves||[])){
    if(p.movePP[mv]===undefined)
      p.movePP[mv]=(MOVE_DATA[mv]&&MOVE_DATA[mv].pp)||20;
  }
}
function restoreMovePP(p){
  p.movePP={};
  for(const mv of (p.moves||[]))
    p.movePP[mv]=(MOVE_DATA[mv]&&MOVE_DATA[mv].pp)||20;
}
...
const BUILDINGS_BY_MAP={
  ...
  route2:[
    // Boulders — solid (fully impassable), repositioned asymmetrically
    {x:3, y:3, key:'boulder',         w:2, h:2, solid:true},
    {x:16,y:3, key:'boulder',         w:2, h:2, solid:true},
    {x:2, y:14,key:'boulder',         w:2, h:2, solid:true},
    {x:17,y:14,key:'boulder',         w:2, h:2, solid:true},
    // Dead trees — scattered, not symmetric
    {x:6, y:5, key:'dead_tree',       w:1, h:2, blockBase:true},
    {x:16,y:6, key:'dead_tree',       w:1, h:2, blockBase:true},
    {x:7, y:9, key:'dead_tree',       w:1, h:2, blockBase:true},
    {x:14,y:12,key:'dead_tree',       w:1, h:2, blockBase:true},
    {x:5, y:14,key:'dead_tree',       w:1, h:2, blockBase:true},
    // Toxic puddles
    {x:6, y:8, key:'toxic_puddle',    w:2, h:1},
    {x:13,y:7, key:'toxic_puddle',    w:2, h:1},
    {x:5, y:13,key:'toxic_puddle',    w:2, h:1},
    {x:13,y:14,key:'toxic_puddle',    w:2, h:1},
    {x:10,y:17,key:'toxic_puddle',    w:2, h:1},
    // SYNTEK billboard — right side, solid, menacing
    {x:17,y:7, key:'syntek_billboard',w:2, h:2, solid:true},
    // Metal crates — break up the right open space, force routing
    {x:12,y:11,key:'metal_crate',     w:2, h:2, solid:true},
    // Signs
    {x:8, y:4, key:'sign_post',       w:1, h:1},
    {x:11,y:10,key:'sign_post',       w:1, h:1},
  ],
  ironhaven:[
...
function drawTile(ctx,t,tx,ty,ts){
  ...
  // Water ripple animation (only drawn when waterPhase is set by rAF loop)
  if(t===TILE.WATER){
    const r1=Math.sin(waterPhase+tx*0.25)*0.09+0.09;
    const r2=Math.sin(waterPhase*1.4+ty*0.2+2.0)*0.06+0.06;
    ctx.fillStyle=`rgba(30,160,255,${r1.toFixed(2)})`;
    ctx.fillRect(tx,ty,ts,ts);
    ctx.fillStyle=`rgba(120,220,255,${r2.toFixed(2)})`;
    ctx.fillRect(tx,ty,ts,ts);
  }
}
...
function drawMap(){
  ...
  for(let ni=0;ni<npcs.length;ni++){
    const npc=npcs[ni];
    const tx=npc.x*TILE_SIZE+offX;
    // NPC idle bob: each NPC gets a different phase so they don't all sync
    const bobOffset=Math.sin(now/600+ni*1.9)*2.5;
    const ty=npc.y*TILE_SIZE+offY+bobOffset;
    if(tx>canvas.width||ty>canvas.height||tx<-TILE_SIZE||ty<-TILE_SIZE)continue;
    // Signs use sign_post building image — no bob
    if(npc.emoji==='📋'){
      const signTy=npc.y*TILE_SIZE+offY; // signs don't bob
      ...
      if(signImg&&signImg.complete&&signImg.naturalWidth>0){
        ctx.drawImage(signImg,tx,signTy,TILE_SIZE,TILE_SIZE);
      } else {
        ctx.font=Math.floor(TILE_SIZE*.7)+'px serif';
        ctx.textAlign='center';ctx.textBaseline='middle';
        ctx.fillText('📋',tx+TILE_SIZE/2,signTy+TILE_SIZE/2);
      }
      ctx.shadowBlur=0;
      continue;
    }
  ...
  // Walk bob: alternates up slightly each step
  const walkBob=(playerWalkFrame%2===1)?-Math.floor(TILE_SIZE*0.1):0;
  ctx.shadowColor='#00ffff';ctx.shadowBlur=12;
  ctx.save();
  // Step 1: move origin to tile position (with bob)
  ctx.translate(px, py+walkBob);
  // Step 2: apply directional flip from (0,0) in local space
  const facing=G.facing||'down';
  if(facing==='left'){
  ...
  // Step 3: always draw at local (0,0) — never use absolute px/py here
  if(typeof CHAR_SPRITES!=='undefined'&&CHAR_SPRITES.player&&CHAR_SPRITES.player.complete&&CHAR_SPRITES.player.naturalWidth>0){
    ctx.drawImage(CHAR_SPRITES.player,0,0,TILE_SIZE,TILE_SIZE);
  } else {
    const FACING_EMOJI={'up':'🔼','down':'🔽','left':'◀️','right':'▶️'};
    ctx.font=Math.floor(TILE_SIZE*.65)+'px serif';
    ctx.textAlign='center';ctx.textBaseline='middle';
    ctx.fillText(FACING_EMOJI[facing]||'🧑',TILE_SIZE/2,TILE_SIZE/2);
  }
  ctx.restore();
  ctx.shadowBlur=0;
}
...
function move(dir){
  ...
  G.facing=dir; // update facing even if blocked, so player turns in place
  playerWalkFrame++; // drives walk bob animation
...
function healParty(){
  G.party.forEach(p=>{p.hp=p.maxHp;restoreMovePP(p);});
  updateOWHeader();
    ...
    if(bagContext==='overworld'){
      div.onclick=()=>{
        if(key==='healpack'){
          ...
          if(!w){notify('All Pokémon are at full HP!');return;}
          w.hp=w.maxHp;restoreMovePP(w);G.bag.fullheal--;updateOWHeader();notify(w.name+' fully restored! ✨','#00ff88');renderBag();
        } else if(key==='revive'){
...
function triggerWildBattle(){
  ...
  wildEncounterFlash(()=>startBattle(wild,false,false));
}
...
function startBattle(enemy,isTrainer,isGym){
  ...
  initMovePP(player); // ensure PP is tracked before copying into battleState
  battleState={
    player:JSON.parse(JSON.stringify(player)),
    enemy:JSON.parse(JSON.stringify(enemy)),
    isTrainer,isGym,turn:'player',over:false,protecting:false,
    playerStatusDef:0, enemyStatusDef:0,
    playerStatus:null, enemyStatus:null,
...
function calcDamage(atk,moveName,def){
  ...
  // DEF bonus: each +1 stage adds 50% more defence (tracked per side)
  const isPlayer=(def===bs.player);
  const defStages=isPlayer?(bs.playerStatusDef||0):(bs.enemyStatusDef||0);
...
function showMoveMenu(){
  ...
  const movePP=battleState.player.movePP||{};
  const allEmpty=battleState.player.moves.every(mv=>(movePP[mv]||0)<=0);

  if(allEmpty){
    // All PP gone — only Struggle available
    const btn=document.createElement('button');btn.className='move-btn';
    btn.innerHTML=`STRUGGLE <span class="move-type type-normal">Normal | PWR:50 | Recoil</span>`;
    btn.onclick=()=>executePlayerMove('Struggle');
  ...
  for(const mv of battleState.player.moves){
    ...
    const maxPP=(MOVE_DATA[mv]&&MOVE_DATA[mv].pp)||20;
    const curPP=movePP[mv]!==undefined?movePP[mv]:maxPP;
    const noPP=curPP<=0;
    const btn=document.createElement('button');btn.className='move-btn';
    ...
    const ppCol=curPP<=Math.floor(maxPP/4)?'#ff4444':curPP<=Math.floor(maxPP/2)?'#ffaa44':'#88dd88';
    btn.innerHTML=`${mv} ${effTag}<span class="move-type ${typeClass}">${md.type} | PWR:${md.pwr||'—'} | <span style="color:${ppCol}">${curPP}/${maxPP} PP</span></span>`;
    if(noPP){btn.disabled=true;btn.style.opacity='0.35';btn.style.cursor='not-allowed';}
    else btn.onclick=()=>executePlayerMove(mv);
    grid.appendChild(btn);
  }
}
function hideMoveMenu(){showMainMenu();}

// ── EXECUTE PLAYER MOVE (v3.5) ──
function executePlayerMove(moveName){
  ...
  // PP decrement (Struggle has pp:1 and is self-resetting — never runs dry)
  if(moveName!=='Struggle'){
    if(!bs.player.movePP) bs.player.movePP={};
    const maxPP=(MOVE_DATA[moveName]&&MOVE_DATA[moveName].pp)||20;
    if(bs.player.movePP[moveName]===undefined) bs.player.movePP[moveName]=maxPP;
    bs.player.movePP[moveName]=Math.max(0,bs.player.movePP[moveName]-1);
    // Sync back to the party member so PP persists after battle
    if(G.party[0]&&G.party[0].movePP)
      G.party[0].movePP[moveName]=bs.player.movePP[moveName];
  }

  // Flinch check (enemy caused player to flinch last turn)
  if(bs.playerStatusFlinch){
    bs.playerStatusFlinch=false;
    setBattleLog(bs.player.name+' flinched and couldn\'t move! 😵');
    setTimeout(()=>{
  ...
  // Confusion check (33% self-hit, wears off after confTurns)
  if(bs.playerStatus&&bs.playerStatus.type==='conf'){
    bs.playerStatus.confTurns=(bs.playerStatus.confTurns||1)-1;
    if(bs.playerStatus.confTurns<=0){
      bs.playerStatus=null;
      setBattleLog(bs.player.name+' snapped out of confusion!');
      setTimeout(()=>_doPlayerAttack(moveName,md),BATTLE_FAST);
      return;
    }
    if(Math.random()<.33){
...
function _doEnemyAttack(afterCb){
  ...
  // Confusion
  if(bs.enemyStatus&&bs.enemyStatus.type==='conf'){
    bs.enemyStatus.confTurns=(bs.enemyStatus.confTurns||1)-1;
    if(bs.enemyStatus.confTurns<=0){
      bs.enemyStatus=null;
      setBattleLog(bs.enemy.name+' snapped out of confusion!');
      setTimeout(()=>{if(afterCb)afterCb();},BATTLE_FAST);
      return;
    }
    if(Math.random()<.33){
...
function endBattle(won,caught){
  ...
  if(G.party[0]){
    ...
    if(bs.player.movePP) G.party[0].movePP=bs.player.movePP; // persist spent PP
  }
  if(won&&!caught){
    ...
    if(Math.random()<catchChance){
      ...
      setTimeout(()=>{
        if(G.party.length<6){
          ...
          initMovePP(c); // give it fresh PP tracking
...
function openBagInBattle(){
  ...
  if((G.bag.fullheal||0)>0){
    ...
    btn.onclick=()=>{
      ...
      restoreMovePP(bs.player);G.party[0].movePP=bs.player.movePP;
  ...
  if((G.bag.revive||0)>0){
    ...
    if(fainted){
      const btn=document.createElement('button');btn.className='move-btn';
      btn.innerHTML=`💫 REVIVE ×${G.bag.revive} <span class="move-type">Revive ${fainted.name} to 50% HP</span>`;
      btn.onclick=()=>{
        fainted.hp=Math.floor(fainted.maxHp*.5);
        G.bag.revive--;
        setBattleLog(fainted.name+' was revived! 💫');
        hideBagInBattle();setTimeout(enemyTurn,BATTLE_FAST);
      };opts.appendChild(btn);
    }
  }
...
function doSwitch(partyIndex){
  const bs=battleState;
  G.party[0].hp=bs.player.hp;
  if(bs.player.movePP) G.party[0].movePP=bs.player.movePP; // persist PP of outgoing mon
