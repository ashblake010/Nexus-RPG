...
function drawMap(){
  ...
  if(facing==='left'){
    // Flip horizontally: translate to right edge of tile, scale X=-1
    ctx.translate(px+TILE_SIZE,py);
    ctx.scale(-1,1);
    ctx.translate(-px,0);
  } else if(facing==='up'){
    // Flip vertically: translate to bottom edge of tile, scale Y=-1
    ctx.translate(px,py+TILE_SIZE);
    ctx.scale(1,-1);
    ctx.translate(0,-py);
  }
  // 'right' and 'down' draw normally
  if(typeof CHAR_SPRITES!=='undefined'&&CHAR_SPRITES.player&&CHAR_SPRITES.player.complete&&CHAR_SPRITES.player.naturalWidth>0){
    ...
    ctx.textAlign='center';ctx.textBaseline='middle';
    ctx.fillText(FACING_EMOJI[facing]||'🧑',px+TILE_SIZE/2,py+TILE_SIZE/2);
  }
  ctx.restore();
  ctx.shadowBlur=0;
}
