...
function startGame(){
  Object.assign(G,{playerName:'ALEX',starter:null,party:[],badges:[],location:'BOOTVILLE',
    steps:0,x:7,y:8,map:'bootville',bag:{nexball:5,superball:0,healpack:3,revive:1,ether:0,maxEther:0,elixir:0,maxElixir:0},
...
function initMovePP(p){
  if(!p.ppCurrent||p.ppCurrent.length!==p.moves.length){
    p.ppCurrent=p.moves.map(mv=>(MOVE_DATA[mv]||{pp:20}).pp);
  }
}
function restoreMovePP(p){
  p.ppCurrent=p.moves.map(mv=>(MOVE_DATA[mv]||{pp:20}).pp);
}
function restoreOneMovePP(p,idx,amount){
  if(!p.ppCurrent)initMovePP(p);
  const maxPP=(MOVE_DATA[p.moves[idx]]||{pp:20}).pp;
  p.ppCurrent[idx]=amount===null?maxPP:Math.min(maxPP,(p.ppCurrent[idx]||0)+amount);
}
function restoreAllMovePP(p,amount){
  if(!p.ppCurrent)initMovePP(p);
  p.moves.forEach((_,i)=>restoreOneMovePP(p,i,amount));
}
// Mark a dex entry (seen or caught)
function dexMark(name,type){
  if(!G.seen)G.seen=[];if(!G.caught)G.caught=[];
  const id=name.toLowerCase().replace(/\s+/g,'');
  // find key in POKEMON_BASE
  const key=Object.keys(POKEMON_BASE).find(k=>POKEMON_BASE[k].name===name)||id;
  if(!G.seen.includes(key))G.seen.push(key);
  if(type==='caught'&&!G.caught.includes(key))G.caught.push(key);
}
...
function loadGame(){
  ...
  if(!G.seen)G.seen=[];
  if(!G.caught)G.caught=[];
  G.party.forEach(p=>initMovePP(p));
...
// ── POKÉDEX ──
// Ordered list of all obtainable Pokémon (no gym-exclusives)
const POKEDEX_ORDER = [
  'sproutex','thornex','vegithorn',
  'embrit','flamcore','infernox',
  'aquibit','hydrobit','tidalcore',
  'glitchling','glitchwraith',
  'coglet','gearoth',
  'sparkit','voltfang',
  'fumewing',
  'nullbot',
...
function openPokedex(){renderPokedex();showScreen('pokedex');}
function closePokedex(){showScreen('overworld');drawMap();}
function renderPokedex(){
  if(!G.seen)G.seen=[];if(!G.caught)G.caught=[];
  const seenCount=G.seen.filter(k=>POKEDEX_ORDER.includes(k)).length;
  const caughtCount=G.caught.filter(k=>POKEDEX_ORDER.includes(k)).length;
  document.getElementById('dex-stats').textContent=`Seen: ${seenCount} / ${POKEDEX_ORDER.length}   Caught: ${caughtCount} / ${POKEDEX_ORDER.length}`;
  const list=document.getElementById('dex-list');list.innerHTML='';
  POKEDEX_ORDER.forEach((key,num)=>{
    const seen=G.seen.includes(key);
    const caught=G.caught.includes(key);
    const base=POKEMON_BASE[key];
    const div=document.createElement('div');
    div.className='dex-entry'+(caught?' dex-caught':seen?' dex-seen':'');
    if(seen&&base){
      const spd=caught?
        `<div class="dex-stats">HP:${base.maxHp} ATK:${base.atk} DEF:${base.def} SPD:${base.spd}</div>`:'';
      div.innerHTML=`
        <div class="dex-num">#${String(num+1).padStart(3,'0')}</div>
        <div class="dex-emoji">${base.emoji}</div>
        <div class="dex-info">
          <div class="dex-name">${base.name} ${caught?'◈':''}</div>
          <div class="dex-type">${base.type}</div>
          ${spd}
        </div>`;
    } else {
      div.innerHTML=`
        <div class="dex-num">#${String(num+1).padStart(3,'0')}</div>
        <div class="dex-emoji" style="opacity:0.2">?</div>
        <div class="dex-info">
          <div class="dex-name" style="color:var(--textdim)">— — — — —</div>
          <div class="dex-type" style="color:var(--textdim)">???</div>
        </div>`;
    }
...
function openMenu(){
  const opts=[
    ...
    {label:'📖 POKÉDEX',action:openPokedex},
...
function renderBag(){
  const items={
    ...
    ether:    {emoji:'🔷',name:'ETHER',      desc:'+10 PP to the move with lowest PP.'},
    maxEther: {emoji:'🔹',name:'MAX ETHER',  desc:'Fully restores PP of one move.'},
    elixir:   {emoji:'🟣',name:'ELIXIR',     desc:'+10 PP to all moves.'},
    maxElixir:{emoji:'💜',name:'MAX ELIXIR', desc:'Fully restores PP of all moves.'},
  };
    ...
    if(bagContext==='overworld'){
      div.onclick=()=>{
        if(key==='healpack'){
          ...
          const w=G.party.find(p=>p.hp>0);if(!w){notify('No able Pokémon!');return;}
          initMovePP(w);const _i=w.ppCurrent.reduce((l,v,i,a)=>v<a[l]?i:l,0);
          restoreOneMovePP(w,_i,10);G.bag.ether--;notify(w.moves[_i]+' PP +10! 🔷','#44aaff');renderBag();
        } else if(key==='maxEther'){
          const w=G.party.find(p=>p.hp>0);if(!w){notify('No able Pokémon!');return;}
          initMovePP(w);const _i=w.ppCurrent.reduce((l,v,i,a)=>v<a[l]?i:l,0);
          restoreOneMovePP(w,_i,null);G.bag.maxEther--;notify(w.moves[_i]+' PP fully restored! 🔹','#44aaff');renderBag();
        } else if(key==='elixir'){
          const w=G.party.find(p=>p.hp>0);if(!w){notify('No able Pokémon!');return;}
          restoreAllMovePP(w,10);G.bag.elixir--;notify('All move PP +10! 🟣','#aa44ff');renderBag();
        } else if(key==='maxElixir'){
          const w=G.party.find(p=>p.hp>0);if(!w){notify('No able Pokémon!');return;}
          restoreAllMovePP(w,null);G.bag.maxElixir--;notify('All move PP fully restored! 💜','#aa44ff');renderBag();
...
function triggerWildBattle(){
  ...
  // Mark seen in dex
  if(!G.seen)G.seen=[];
  if(!G.seen.includes(wildId))G.seen.push(wildId);
...
function showMoveMenu(){
  ...
  const bs=battleState;
  initMovePP(bs.player);
  const allEmpty=bs.player.ppCurrent.every(pp=>pp<=0);
  if(allEmpty){
    const btn=document.createElement('button');btn.className='move-btn';
    btn.innerHTML=`STRUGGLE <span class="move-type type-normal">Normal | PWR:50 — Recoil 25%</span>`;
    btn.onclick=()=>executePlayerMove('Struggle');grid.appendChild(btn);
    return;
  }
  bs.player.moves.forEach((mv,idx)=>{
    const md=MOVE_DATA[mv]||{pwr:50,type:'Normal',pp:20};
    const curPP=bs.player.ppCurrent[idx]??md.pp;
    const maxPP=md.pp||20;
    const typeClass='type-'+md.type.toLowerCase().split('/')[0];
    const mult=getTypeMultiplier(md.type||'Normal',bs.enemy.type||'Normal');
    const effTag=mult>=2?'<span style="color:#ff4">▲SE</span>':mult===0?'<span style="color:#888">✕</span>':mult<1?'<span style="color:#f80">▼NVE</span>':'';
    const ppCol=curPP/maxPP>0.5?'#0f0':curPP/maxPP>0.25?'#ff0':'#f55';
    const btn=document.createElement('button');btn.className='move-btn';
    btn.innerHTML=`${mv} ${effTag}<span class="move-type ${typeClass}">${md.type} | PWR:${md.pwr||'—'}</span><span style="color:${ppCol};font-size:10px;margin-left:4px;">${curPP}/${maxPP}</span>`;
    if(curPP<=0)btn.disabled=true;
...
function executePlayerMove(moveName){
  ...
  document.getElementById('battle-actions').style.display='none';
  document.getElementById('battle-moves').style.display='none';
  // Decrement PP (skip Struggle)
  if(moveName!=='Struggle'){
    initMovePP(bs.player);
    const _ppIdx=bs.player.moves.indexOf(moveName);
    if(_ppIdx>=0&&bs.player.ppCurrent[_ppIdx]>0){
      bs.player.ppCurrent[_ppIdx]--;
      if(G.party[0]){initMovePP(G.party[0]);if(G.party[0].moves[_ppIdx]===moveName)G.party[0].ppCurrent[_ppIdx]=bs.player.ppCurrent[_ppIdx];}
    }
  }
...
function openBagInBattle(){
  ...
  // Ether – restores 10 PP to move with lowest PP
  if((G.bag.ether||0)>0){
    const btn=document.createElement('button');btn.className='move-btn';
    btn.innerHTML=`🔷 ETHER ×${G.bag.ether} <span class="move-type">+10 PP to lowest-PP move</span>`;
    btn.onclick=()=>{
      const bs=battleState;initMovePP(bs.player);
      const idx=bs.player.ppCurrent.reduce((low,v,i,a)=>v<a[low]?i:low,0);
      restoreOneMovePP(bs.player,idx,10);
      if(G.party[0]){initMovePP(G.party[0]);G.party[0].ppCurrent[idx]=bs.player.ppCurrent[idx];}
      G.bag.ether--;setBattleLog(bs.player.moves[idx]+' PP restored!');
      hideBagInBattle();setTimeout(enemyTurn,BATTLE_FAST);
    };opts.appendChild(btn);
  }
  // Max Ether – fully restores PP of move with lowest PP
  if((G.bag.maxEther||0)>0){
    const btn=document.createElement('button');btn.className='move-btn';
    btn.innerHTML=`🔹 MAX ETHER ×${G.bag.maxEther} <span class="move-type">Full PP to lowest-PP move</span>`;
    btn.onclick=()=>{
      const bs=battleState;initMovePP(bs.player);
      const idx=bs.player.ppCurrent.reduce((low,v,i,a)=>v<a[low]?i:low,0);
      restoreOneMovePP(bs.player,idx,null);
      if(G.party[0]){initMovePP(G.party[0]);G.party[0].ppCurrent[idx]=bs.player.ppCurrent[idx];}
      G.bag.maxEther--;setBattleLog(bs.player.moves[idx]+' PP fully restored!');
      hideBagInBattle();setTimeout(enemyTurn,BATTLE_FAST);
    };opts.appendChild(btn);
  }
  ...
  if((G.bag.elixir||0)>0){
    const btn=document.createElement('button');btn.className='move-btn';
    btn.innerHTML=`🟣 ELIXIR ×${G.bag.elixir} <span class="move-type">+10 PP to all moves</span>`;
    btn.onclick=()=>{
      const bs=battleState;restoreAllMovePP(bs.player,10);
      if(G.party[0])restoreAllMovePP(G.party[0],10);
      G.bag.elixir--;setBattleLog('All move PP restored by 10!');
      hideBagInBattle();setTimeout(enemyTurn,BATTLE_FAST);
    };opts.appendChild(btn);
  }
  // Max Elixir – fully restores all move PP
  if((G.bag.maxElixir||0)>0){
    const btn=document.createElement('button');btn.className='move-btn';
    btn.innerHTML=`💜 MAX ELIXIR ×${G.bag.maxElixir} <span class="move-type">Full PP to all moves</span>`;
    btn.onclick=()=>{
      const bs=battleState;restoreAllMovePP(bs.player,null);
      if(G.party[0])restoreAllMovePP(G.party[0],null);
      G.bag.maxElixir--;setBattleLog('All move PP fully restored!');
      hideBagInBattle();setTimeout(enemyTurn,BATTLE_FAST);
    };opts.appendChild(btn);
  }
