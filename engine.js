...
function drawTile(ctx,t,tx,ty,ts){
  const c=TILE_COLOR[t]||'#111';
  const b=TILE_BORDER[t]||'#222';
  const h=Math.floor(ts/2);
  ctx.fillStyle=c;
  ctx.fillRect(tx,ty,ts,ts);

  // Pixel art tile details
  if(t===TILE.GRASS){
    // grass texture - lighter patches
    ctx.fillStyle='#122212';
    ctx.fillRect(tx+2,ty+2,3,2);ctx.fillRect(tx+ts-6,ty+4,3,2);
    ctx.fillRect(tx+4,ty+ts-5,2,3);ctx.fillRect(tx+ts-4,ty+ts-4,2,2);
    ctx.strokeStyle=b;ctx.lineWidth=1;ctx.strokeRect(tx+.5,ty+.5,ts-1,ts-1);
  } else if(t===TILE.PATH){
    // asphalt path - pixel cracks
    ctx.fillStyle='#1c1c16';
    ctx.fillRect(tx+3,ty+h,ts-6,1);
    ctx.fillStyle='#0e0e0a';
    ctx.fillRect(tx+h,ty+3,1,ts-6);
    ctx.strokeStyle='#2a2a20';ctx.lineWidth=1;ctx.strokeRect(tx+.5,ty+.5,ts-1,ts-1);
    // neon road line
    if((tx/ts+ty/ts)%4===0){
      ctx.fillStyle='#ffdd0022';ctx.fillRect(tx+h-1,ty+1,2,ts-2);
    }
  } else if(t===TILE.WALL){
    // city wall - brick pattern
    const row=Math.floor(ty/ts)%2;
    ctx.fillStyle='#0e1828';
    for(let i=0;i<3;i++){
      const bx=tx+(row?0:Math.floor(ts/3))+i*Math.floor(ts/1.5)%(ts);
      ctx.fillRect(bx,ty+2,Math.floor(ts/3)-1,Math.floor(ts/2)-2);
      ctx.fillRect(bx+Math.floor(ts/6),ty+Math.floor(ts/2)+1,Math.floor(ts/3)-1,Math.floor(ts/2)-2);
    }
    // neon accent top edge
    ctx.fillStyle='#00ffff18';ctx.fillRect(tx,ty,ts,2);
    ctx.strokeStyle=b;ctx.lineWidth=1;ctx.strokeRect(tx+.5,ty+.5,ts-1,ts-1);
  } else if(t===TILE.TALL){
    // tall grass - pixel blades
    ctx.fillStyle='#174817';
    for(let i=0;i<5;i++){
      const gx=tx+3+i*(Math.floor(ts/5));
      ctx.fillRect(gx,ty+4,2,Math.floor(ts*.55));
      ctx.fillRect(gx+1,ty+2,1,3);
    }
    ctx.fillStyle='#1f6a1f';
    for(let i=0;i<4;i++){
      const gx=tx+5+i*(Math.floor(ts/4));
      ctx.fillRect(gx,ty+6,1,Math.floor(ts*.4));
    }
    ctx.strokeStyle=b;ctx.lineWidth=1;ctx.strokeRect(tx+.5,ty+.5,ts-1,ts-1);
  } else if(t===TILE.WATER){
    // water - animated shimmer via position-based offset
    const wave=(Math.floor(tx/ts+ty/ts))%3;
    ctx.fillStyle='#0a1a30';
    ctx.fillRect(tx+2,ty+h-1+wave,ts-4,2);
    ctx.fillStyle='#0d2040';
    ctx.fillRect(tx+4,ty+h+3+wave,ts-8,1);
    // cyan shimmer
    ctx.fillStyle='#00ffff15';
    ctx.fillRect(tx+h-2,ty+3,4,2);
    ctx.strokeStyle=b;ctx.lineWidth=1;ctx.strokeRect(tx+.5,ty+.5,ts-1,ts-1);
  } else if(t===TILE.CENTER){
    // pokecenter - pink neon cross
    ctx.fillStyle='#200a30';ctx.fillRect(tx,ty,ts,ts);
    ctx.fillStyle='#ff88cc';
    ctx.fillRect(tx+h-1,ty+4,3,ts-8);
    ctx.fillRect(tx+4,ty+h-1,ts-8,3);
    // neon glow border
    ctx.strokeStyle='#ff88cc66';ctx.lineWidth=2;ctx.strokeRect(tx+1,ty+1,ts-2,ts-2);
    ctx.font=Math.floor(ts*.4)+'px serif';ctx.textAlign='center';ctx.textBaseline='middle';
    ctx.fillText('🏥',tx+h,ty+h);
  } else if(t===TILE.GYM){
    // gym - red neon border
    ctx.fillStyle='#200808';ctx.fillRect(tx,ty,ts,ts);
    ctx.strokeStyle='#ff334488';ctx.lineWidth=2;ctx.strokeRect(tx+1,ty+1,ts-2,ts-2);
    ctx.strokeStyle='#ff000033';ctx.lineWidth=1;ctx.strokeRect(tx+3,ty+3,ts-6,ts-6);
    ctx.font=Math.floor(ts*.5)+'px serif';ctx.textAlign='center';ctx.textBaseline='middle';
    ctx.fillText('🏟',tx+h,ty+h);
  } else if(t===TILE.TREE){
    // pixel tree - dark trunk, layered canopy
    ctx.fillStyle='#4a2800';ctx.fillRect(tx+h-2,ty+Math.floor(ts*.6),4,Math.floor(ts*.4));
    ctx.fillStyle='#0d3a0d';ctx.fillRect(tx+4,ty+4,ts-8,Math.floor(ts*.65));
    ctx.fillStyle='#155515';ctx.fillRect(tx+5,ty+3,ts-10,Math.floor(ts*.5));
    ctx.fillStyle='#1e7a1e';ctx.fillRect(tx+h-3,ty+2,6,Math.floor(ts*.35));
    ctx.strokeStyle=b;ctx.lineWidth=1;ctx.strokeRect(tx+.5,ty+.5,ts-1,ts-1);
  } else if(t===TILE.SIGN){
    // sign - wooden post pixel art
    ctx.fillStyle='#3a2a10';ctx.fillRect(tx+h-1,ty+h,2,h-2);
    ctx.fillStyle='#5a4520';ctx.fillRect(tx+4,ty+4,ts-8,h-2);
    ctx.fillStyle='#7a6030';ctx.fillRect(tx+5,ty+5,ts-10,h-4);
    ctx.font=Math.floor(ts*.35)+'px serif';ctx.textAlign='center';ctx.textBaseline='middle';
    ctx.fillText('!',tx+h,ty+Math.floor(ts*.35));
    ctx.strokeStyle=b;ctx.lineWidth=1;ctx.strokeRect(tx+.5,ty+.5,ts-1,ts-1);
  } else if(t===TILE.EXIT_N||t===TILE.EXIT_S){
    // exit portal - animated cyan
    ctx.fillStyle='#00ffff08';ctx.fillRect(tx,ty,ts,ts);
    ctx.strokeStyle='#00ffff55';ctx.lineWidth=1;ctx.strokeRect(tx+2,ty+2,ts-4,ts-4);
    ctx.fillStyle='#00ffff22';ctx.fillRect(tx+4,ty+4,ts-8,ts-8);
    // arrow
    ctx.fillStyle='#00ffff88';
    ctx.font=Math.floor(ts*.5)+'px monospace';ctx.textAlign='center';ctx.textBaseline='middle';
    ctx.fillText(t===TILE.EXIT_N?'↑':'↓',tx+h,ty+h);
  } else {
    ctx.strokeStyle=b;ctx.lineWidth=1;ctx.strokeRect(tx+.5,ty+.5,ts-1,ts-1);
  }
}
...
function drawMap(){
  ...
  ctx.strokeStyle='#00ffff66';ctx.lineWidth=2;
