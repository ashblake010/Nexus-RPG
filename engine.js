...
function startGame(){
  Object.assign(G,{playerName:'ALEX',starter:null,party:[],badges:[],location:'BOOTVILLE',
    ...
    money:500,flags:{metAda:false,beatGym1:false,visitedGridlock:false,beatKael:false,visitedIronhaven:false,beatGym2:false}});
  runIntro();
}
...
function initMovePP(p){
  ...
  for(const mv of (p.moves||[])){
    if(p.movePP[mv]===undefined) p.movePP[mv]=(MOVE_DATA[mv]||{pp:20}).pp;
  }
}
function resetMovePP(p){
  p.movePP={};
  for(const mv of (p.moves||[])) p.movePP[mv]=(MOVE_DATA[mv]||{pp:20}).pp;
}
...
function initOverworld(){
  G.party.forEach(initMovePP); // ensure PP is initialised for all party members
...
function healParty(){
  G.party.forEach(p=>{p.hp=p.maxHp;resetMovePP(p);});
  updateOWHeader();
...
function startBattle(enemy,isTrainer,isGym){
  ...
  initMovePP(battleState.player); // always ensure PP is ready
...
function showMoveMenu(){
  ...
  const bs=battleState;
  if(!bs.player.movePP) initMovePP(bs.player);
  // Check if all moves are out of PP — force Struggle
  const allEmpty=bs.player.moves.every(mv=>(bs.player.movePP[mv]||0)<=0);
  if(allEmpty){
    const btn=document.createElement('button');btn.className='move-btn';
    btn.innerHTML=`STRUGGLE <span class="move-type type-normal">Normal | PWR:50</span>`;
    btn.onclick=()=>executePlayerMove('Struggle');grid.appendChild(btn);
    return;
  }
  for(const mv of bs.player.moves){
    const md=MOVE_DATA[mv]||{pwr:50,type:'Normal',pp:20};
    const curPP=bs.player.movePP[mv]!==undefined?bs.player.movePP[mv]:md.pp;
    const maxPP=md.pp||20;
    const noPP=curPP<=0;
    const btn=document.createElement('button');btn.className='move-btn'+(noPP?' pp-empty':'');
    ...
    const ppCol=curPP<=Math.floor(maxPP/4)?'color:#ff4444':curPP<=Math.floor(maxPP/2)?'color:#ff8800':'color:#aaffaa';
    btn.innerHTML=`${mv} ${effTag}<span class="move-type ${typeClass}">${md.type} | PWR:${md.pwr||'—'}</span><span style="font-size:10px;${ppCol};margin-left:4px;">${noPP?'NO PP':curPP+'/'+maxPP}</span>`;
    if(noPP) btn.disabled=true,btn.title='No PP left!';
...
function executePlayerMove(moveName){
  ...
  // Deduct PP (skip for Struggle which has pp:999)
  if(moveName!=='Struggle'){
    if(!bs.player.movePP) initMovePP(bs.player);
    if(bs.player.movePP[moveName]>0) bs.player.movePP[moveName]--;
    // Sync PP back to the party lead so it persists after battle
    const partyLead=G.party.find(p=>p.name===bs.player.name);
    if(partyLead){
      if(!partyLead.movePP) partyLead.movePP={};
      partyLead.movePP[moveName]=bs.player.movePP[moveName];
    }
  }
...
function endBattle(won,caught){
  ...
  if(G.party[0]){
    ...
    // Sync PP from battle back to party
    if(bs.player.movePP) G.party[0].movePP=JSON.parse(JSON.stringify(bs.player.movePP));
  }
  if(won&&!caught){
...
function doSwitch(partyIndex){
  const bs=battleState;
  // Sync HP and PP of outgoing Pokémon back to party slot 0
  G.party[0].hp=bs.player.hp;
  if(bs.player.movePP) G.party[0].movePP=JSON.parse(JSON.stringify(bs.player.movePP));
  const chosen=G.party.splice(partyIndex,1)[0];
  G.party.unshift(chosen);
  bs.player=JSON.parse(JSON.stringify(chosen));
  initMovePP(bs.player); // ensure PP is set if this is a freshly-caught mon
